...
static bool mgos_telegram_acl_check(uint32_t chat_id) {
  ...
  if (res) {
    LOG(LL_DEBUG, ("TELEGRAM ->> mgos_telegram_acl_check() ->> Chat ID: %lu found in ACL list, accept message", ( unsigned long )chat_id));
  }
  else {
    LOG(LL_INFO, ("TELEGRAM ->> mgos_telegram_acl_check() ->> User or Chat: %lu not found in ACL list, ignore message", ( unsigned long )chat_id));
  }
  ...
  switch (el->method) {
    ...
    case sendMessage: {
      ...
      if (el->data != NULL) {
        ...
        LOG(LL_INFO, ("TELEGRAM ->> Send message to User or Chat: %li, Message: %s", ( long )el->chat_id, el->data));
      }
