...
static bool mgos_telegram_acl_check(uint32_t chat_id) {
  ...
  if (res) {
    ...
    LOG(LL_INFO, ("TELEGRAM ->> mgos_telegram_acl_check() ->> Chat ID: %lu not found in ACL list, ignore message", ( unsigned long )chat_id));
  }
  ...
  switch (el->method) {
    ...
    case sendMessage: {
      ...
      if (el->data != NULL) {
        ...
        LOG(LL_INFO, ("TELEGRAM ->> send message to User/Chat: %li, Message: %s", ( long )el->chat_id, el->data));
      }
