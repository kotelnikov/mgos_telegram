...
`json_data` | Вы также можете отправить строку в формате JSON, содержащую атрибуты описанные в Telegram Bot API для метода SendMessage, см. описание:  https://core.telegram.org/bots/api#sendmessage, для того чтобы использовать родное Telegram Bot API. В таком случае аргументы `chat_id` и `text` можно оставить пустыми строками `''`. Если не требуется использовать родное Telegram Bot API, передайте `null` в качестве этого аргумента.
